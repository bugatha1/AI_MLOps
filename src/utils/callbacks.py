...
def create_and_save_tensorboard_callback(callbacks_dir, tensorboard_log_dir):
    ...
    logging.info(f"tensorboard callback being saved at {tb_callback_filepath}")
...
def create_and_save_checkpoint_callback(callbacks_dir, checkpoint_dir):
    ...
    logging.info(f"checkpoint callback being saved at {ckpt_callback_filepath}")

def get_callbacks(callbacks_dir_path):
    callback_path = [
        os.path.join(callbacks_dir_path, bin_file) for bin_file 
        in os.listdir(callbacks_dir_path) if bin_file.endswith(".cb")
    ]
